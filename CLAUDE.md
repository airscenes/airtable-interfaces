...
- **Excel templates**: `artist_report/frontend/excelTemplate.js` fills a template by tags (`${scalar}`, one-row blocks `${prefix.field}` repeated per record), never by row number; it unshares shared formulas (else ExcelJS throws "Shared Formula master must exist…"), re-points formulas after row insertion and rebuilds merges. Reuse it for any new template-based export
